...
richness <- read.csv("~/Documents/GitHub/Reclamation_biosolid/Datasheets for analysis/richness.csv")
...
HG.rich <- escalc(n2i = Nc, n1i = Ne, m2i = Mc2, m1i = Me,
                   sd2i = Scimp, sd1i = Seimp, data = richness, measure = "SMD",
                  append = TRUE)


ROM.rich <- escalc(n2i = Nc, n1i = Ne, m2i = Mc2, m1i = Me, 
...
ROM.rich$year.log<-log(ROM.rich$year_restoration)
...
shrubland <- which(ROM.rich$biome== "Shrubland")
rich.shrubland <- ROM.rich[shrubland, ]
richeffshrub <- rma.mv(yi = yi, V = vi, random = rand.var, 
                       data = rich.shrubland, method = "ML")
summary(richeffshrub)

##subset: GRASSLAND
# subset of data with only grassland to feed to rma
grassland <- which(ROM.rich$biome== "Temperate grassland")
rich.grassland <- ROM.rich[grassland, ]
richeffgrass <- rma.mv(yi = yi, V = vi, random = rand.var, 
                       data = rich.grassland, method = "ML")
summary(richeffgrass)

##subset: FOREST
forest <- which(ROM.rich$biome== "Temperate seasonal forest")
rich.forest <- ROM.rich[forest, ]
richeffforest<- rma.mv(yi = yi, V = vi, random = rand.var, 
                       data = rich.forest, method = "ML")
summary(richeffforest)
...
rma.lev.r<-rma.mv(yi,vi,mods = ~ level.log,random =rand.var,
                  data = ROM.rich, method ="ML")
lev.dfr<-as.data.frame(coef(summary(rma.lev.r)))

rma.yr.r<-rma.mv(yi,vi,mods = ~ year.log, random =rand.var,
                 data = ROM.rich,method ="ML")
yr.dfr<-as.data.frame(coef(summary(rma.yr.r)))

rma.temp.r<-rma.mv(yi,vi,mods = ~ temp.log, random =rand.var,
                   data = ROM.rich, method ="ML")
temp.dfr<-as.data.frame(coef(summary(rma.temp.r)))

rma.precip.r<-rma.mv(yi,vi,mods = ~ precip.log, random =rand.var,
                     data = ROM.rich, method ="ML")
...
rma.biome.r<-rma.mv(yi, vi, mods = ~1 + biome, random = rand.var, 
                    data = ROM.rich, method = "ML")
biome.dfr<-as.data.frame(coef(summary(rma.biome.r)))

simple.richness.models<- rbind(ran.dfr, lev.dfr, yr.dfr, temp.dfr, precip.dfr, mix.dfr, 
                               dis.dfr, mult.dfr, seed.dfr, biome.dfr)
...
rma.mv(yi,vi,mods = ~ level.log,random =rand.var, data = ROM.rich,
       method ="ML")
rma.mv(yi,vi,mods = ~ year.log, random =rand.var, data = ROM.rich,
       method ="ML")
rma.mv(yi,vi,mods = ~ temp.log, random = rand.var,data = ROM.rich, 
       method ="ML")
rma.mv(yi,vi,mods = ~  precip.log, random =rand.var,data = ROM.rich, 
       method ="ML")
...
rma.mv(yi,vi,mods = ~ Seeded, random =rand.var,
                 data = ROM.rich, method ="ML")
rma.mv(yi, vi, mods = ~biome, random = rand.var, 
                    data = ROM.rich, method = "ML")
...
candidateset1 <- glmulti(yi ~ S.disturbance + multapp + Seeded +
 Mixture + year.log + level.log + biome, V = "vi",
 random = "rand.var", data = ROM.rich, level = 1, method = "d", 
 fitfunction = rma.glmulti, crit = "aicc")

# run glmulti with meta regression function with biome but no temp and precip
richnessmodels1 <- glmulti(yi ~ S.disturbance + multapp + Seeded +
Mixture + year.log + level.log + biome , V = "vi",
 random = "rand.var", data = ROM.rich, level = 1, method = "h", 
...
# run glmulti with meta regression function with temp and precip but no biome
candidateset2 <- glmulti(yi ~ S.disturbance + multapp + Seeded +
                           Mixture + year.log + level.log + temp.log + precip.log, V = "vi",
                         random = "rand.var", data = ROM.rich, level = 1, method = "d", 
                         fitfunction = rma.glmulti, crit = "aicc")


richnessmodels2 <- glmulti(yi ~ S.disturbance + multapp + Seeded +
                             Mixture + year.log + level.log + temp.log + precip.log , V = "vi",
                           random = "rand.var", data = ROM.rich, level = 1, method = "h", 
                           fitfunction = rma.glmulti, crit = "aicc", confsetsize = candidateset2)

print(richnessmodels2)
bestrichnessmodel2<-summary(richnessmodels2@objects[[1]])
plot(richnessmodels2, type="s")
#models with precip and temp are the better suite of models

##glmulti within biomes
candidatesetgrass <- glmulti(yi ~ S.disturbance + multapp + Seeded +
                           Mixture + year.log + level.log + precip.log + temp.log, V = "vi",
                         random = "rand.var", data = rich.grassland, level = 1, method = "d", 
                         fitfunction = rma.glmulti, crit = "aicc")


richnessmodelsgrass <- glmulti(yi ~ S.disturbance + multapp + Seeded +
                             Mixture + year.log + level.log + precip.log + temp.log, V = "vi",
                           random = "rand.var", data = ROM.rich, level = 1, method = "h", 
                           fitfunction = rma.glmulti, crit = "aicc", confsetsize = candidatesetgrass)
plot(richnessmodelsgrass, type="s")

rma.mv(yi, vi, mods = ~ S.disturbance, random = rand.var,
                 data = rich.grassland,slab = paste(author, year))
##forest
candidatesetfor <- glmulti(yi ~ S.disturbance + multapp + Seeded +
                               Mixture + year.log + level.log + temp.log + precip.log, V = "vi",
                             random = "rand.var", data = rich.forest, level = 1, method = "d", 
                             fitfunction = rma.glmulti, crit = "aicc")


richnessmodelsforest<- glmulti(yi ~ S.disturbance + multapp + Seeded +
                                 Mixture + year.log + level.log + temp.log + precip.log , V = "vi",
                               random = "rand.var", data = ROM.rich, level = 1, method = "h", 
                               fitfunction = rma.glmulti, crit = "aicc", confsetsize = candidatesetfor)
print(richnessmodels2)
bestrichnessmodel2<-summary(richnessmodels2@objects[[1]])
plot(richnessmodels2, type="s")
...
#best fit from output (temp, precip model), excluding variables not considered important criteria according to plot
bestrich1<-rma.mv(yi, vi, mods = ~ S.disturbance + precip.log + Seeded, random = rand.var, data = ROM.rich, 
                  slab = paste(author, year))


#subsets of best model removing all correlations greater than 0.3.
#All 4 variables correlated so we will use the single models
bestrich2<-rma.mv(yi, vi, mods = ~ S.disturbance, random = rand.var,
                 data = ROM.rich, slab = paste(author, year))

bestrich3<-rma.mv(yi, vi, mods = ~ Seeded, random = rand.var,
                 data = ROM.rich, slab = paste(author, year))

bestrich4<-rma.mv(yi, vi, mods = ~ precip.log, random = rand.var,
                 data = ROM.rich, slab = paste(author, year))

richnessmodelslist = list(bestrich1, bestrich2, bestrich3, bestrich4)
...
for (i in 1:length(richnessmodelslist)) {
  ...
  pseudo.r2r[i] <- (sum(rma.random.c$sigma2) - sum(richnessmodelslist[[i]]$sigma2)) / sum(rma.random.c$sigma2)
...
# correlations
rich.corr <- hetcor(data = ROM.rich[, c(31:34,37,38, 42:45)], std.err = FALSE)
rich.corr
#model with interactions of top variables
richnessmixed1<-rma.mv(yi, vi, mods = ~ S.disturbance * Seeded, random = rand.var,data = ROM.rich, slab = paste(author, year))

richnessmixed2<-rma.mv(yi, vi, mods = ~ precip.log * multapp, random = rand.var,data = ROM.rich,
                       slab = paste(author, year))

richnessmixed3<-rma.mv(yi, vi, mods = ~ precip.log*Seeded*
                         multapp, random = rand.var,data = ROM.rich,
                       slab = paste(author, year))
