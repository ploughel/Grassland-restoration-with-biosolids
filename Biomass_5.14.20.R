...
Biomass$mc_0 <- ifelse(Biomass$mc ==0, Biomass$mc+1/2 , Biomass$mc)
Biomass$me_0 <- ifelse(Biomass$mc ==0, Biomass$me+1/2 , Biomass$me)


#View(Biomass)
ROM.Biomass <- escalc(n2i = nc, n1i = ne, m2i = mc_0, m1i = me_0, 
...
# 
# names(ROM.Biomass)
# ROM.ai.size <- glmulti(yi ~biosolid_level_mg_ha_1+time+temp+precip+mixture_y_n+burn_y_n+
#                         seeded_y_n+multiple_application_y_n+severe_disturbance+ai, data=ROM.Biomass, method="d",
#                       level=1, crit="aicc", fitfunction = rma.glmulti)
# 
# ROM.ai.mod <- glmulti(yi ~ biosolid_level_mg_ha_1+time+temp+precip+mixture_y_n+burn_y_n+
#                        seeded_y_n+multiple_application_y_n+severe_disturbance+ai, data=ROM.Biomass, method="h",
...
ROM.ai.rma<-rma.mv(yi, vi, mods = ~time+burn_y_n+  temp+ severe_disturbance+mixture_y_n,
                       random = rand.var,data = ROM.Biomass,slab = paste(author, year), method="ML")


#models with interactions

ROM.prod.int1<-rma.mv(yi, vi, mods = ~time*mixture_y_n,
                     random = rand.var,data = ROM.Biomass,slab = paste(author, year), method="ML")

ROM.prod.int2<-rma.mv(yi, vi, mods = ~time*burn_y_n,
                      random = rand.var,data = ROM.Biomass,slab = paste(author, year), method="ML")

ROM.prod.int3<-rma.mv(yi, vi, mods = ~time*severe_disturbance,
                      random = rand.var,data = ROM.Biomass,slab = paste(author, year), method="ML")

ROM.prod.int4<-rma.mv(yi, vi, mods = ~temp*mixture_y_n,
                      random = rand.var,data = ROM.Biomass,slab = paste(author, year), method="ML")

ROM.prod.int5<-rma.mv(yi, vi, mods = ~temp*burn_y_n,
                       random = rand.var,data = ROM.Biomass,slab = paste(author, year), method="ML")

ROM.prod.int6<-rma.mv(yi, vi, mods = ~temp*severe_disturbance,
                       random = rand.var,data = ROM.Biomass,slab = paste(author, year), method="ML")



mods = list(ROM.ai.rma, ROM.ai.rma.avg,ROM.prod.int1,ROM.prod.int2,ROM.prod.int3,ROM.prod.int5,ROM.prod.int6)
...
#subset by burn
prod.int2a.burn<-rma.mv(yi, vi, mods = ~ time, random = rand.var,data = ROM.Biomass,slab = paste(author, year), subset = burn_y_n=="Y", method = "ML")
...
dim(preds.b)
 ...
 prod.int2a.burn.n<-rma.mv(yi, vi, mods = ~ time, random = rand.var,data = ROM.Biomass,slab = paste(author, year), subset = burn_y_n=="N", method = "ML")
 ...
 bubble.time.Burn=ggplot(preds.burn, aes(x=X.time,y=pred))+
  ...
  scale_color_manual(values=c("black","red"))+
   ...
   geom_point(data=ROM.Biomass, aes(x=time, y = yi, size=1/vi, alpha=.5,color=burn_y_n, group=burn_y_n, fill=burn_y_n, shape=burn_y_n))+
   ylab("")+
 ...
 # #subset by seeded
 # 
 # prod.int3a.Seeded<-rma.mv(yi, vi, mods = ~ time, random = rand.var,data = ROM.Biomass,slab = paste(author, year), subset = seeded_y_n=="Y", method = "ML")
 ...
 # 
 # prod.int3a.Seeded.n<-rma.mv(yi, vi, mods = ~ time, random = rand.var,data = ROM.Biomass,slab = paste(author, year), subset = seeded_y_n=="N", method = "ML")
 ...
 #   geom_point(data=ROM.Biomass, aes(x=time, y = yi, size=1/vi, alpha=.5,color=seeded_y_n, group=seeded_y_n, fill=seeded_y_n, shape=seeded_y_n))+
 #   ylab("")+
 #   xlab("")+
